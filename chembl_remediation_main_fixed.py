...
1. Applies schema changes (runs migration via MCP execute_sql)
...
Usage:
    python chembl_remediation_main_fixed.py [--project_id PROJECT_ID] [--skip_steps STEPS]

Arguments:
    --project_id: Supabase project ID (optional, will be auto-detected if not provided)
    --skip_steps: Comma-separated list of steps to skip (e.g., "migration,import")
...
SELECT execute_ddl('CREATE INDEX IF NOT EXISTS idx_molecules_chembl_id ON molecules (chembl_id);') as ddl_result;
    ...
    logger.info(f"Read migration from {migration_file}")
        ...
        if sql.strip().upper().startswith(('SELECT', 'UPDATE', 'INSERT', 'DELETE')):
            ...
            try:
                ...
                if isinstance(result, dict) and "error" in result:
                    logger.error(f"Migration statement {i+1} failed: {result['error']}")
                    return {
                        "status": "error",
                        "message": f"Migration statement {i+1} failed: {result['error']}",
                        "details": {
                            "statement": sql,
                            "result": result
                        }
                ...
                logger.error(f"Migration statement {i+1} failed: {str(e)}")
                return {
                    "status": "error",
                    "message": f"Migration statement {i+1} failed: {str(e)}",
                    "details": {
                        "statement": sql,
                        "exception": str(e)
                    }
...
def verify_migration(project_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    # Check if chembl_id column exists
    column_query = """
    SELECT column_name
    ...
    backfilled_count = backfill_result[0]["count"] if backfill_result else 0
    ...
    try:
        ...
        if isinstance(property_source_result, dict) and "error" in property_source_result:
            ...
            results["property_sources"] = {
                ...
                "chembl_sources_count": sum(int(r["count"]) for r in chembl_sources),
