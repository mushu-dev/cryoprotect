...
podman cp /home/mushu/Projects/CryoProtect/test_property_based_import.py $CONTAINER_NAME:/app/

# Run the test in the container with environment variables
echo "Running property-based PubChem import tests..."
podman exec -e TEST_DB_HOST="$HOST" -e TEST_DB_PORT="$PORT" -e TEST_DB_NAME="$DBNAME" -e TEST_DB_USER="$USER" -e TEST_DB_PASSWORD="$PASSWORD" \
    $CONTAINER_NAME bash -c "cd /app && python -m unittest test_property_based_import.py"
