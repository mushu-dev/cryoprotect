#!/bin/bash
# This script recreates issues from blueprint-house/cryoprotect to mushu-dev/cryoprotect

# Set up variables
SOURCE_REPO="blueprint-house/cryoprotect"
DEST_REPO="mushu-dev/cryoprotect"
LOG_FILE="issue_recreation_$(date +%Y%m%d_%H%M%S).log"
ISSUE_COUNT=0
SUCCESS_COUNT=0
FAIL_COUNT=0

echo "Starting issue recreation from $SOURCE_REPO to $DEST_REPO"
...
# Get all issues (up to 500 which is the max supported by GitHub CLI)
ISSUES=$(gh issue list -s all -L 500 --json number,title,body,state,labels,assignees,milestone -R "$SOURCE_REPO")
...
echo "Found $TOTAL_ISSUES issues to recreate"
echo "Total issues to recreate: $TOTAL_ISSUES" >> "$LOG_FILE"
...
echo "$ISSUES" | jq -c '.[]' | while read -r issue; do
  ...
  echo "Original Issue #$ISSUE_NUMBER in $SOURCE_REPO" >> issue_mapping.txt
  ...
  # If labels is not empty, add to command
  LABEL_CMD=""
  if [ ! -z "$LABELS" ]; then
    LABEL_CMD="--label \"$LABELS\""
  fi
...
*Recreated from original issue blueprint-house/cryoprotect#${ISSUE_NUMBER}*"
  ...
  # Add to mapping file
  echo "  → New Issue #$NEW_ISSUE_NUMBER in $DEST_REPO" >> issue_mapping.txt
  echo "  → $NEW_ISSUE_URL" >> issue_mapping.txt
  echo "" >> issue_mapping.txt
  ...
  if [ ! -z "$NEW_ISSUE_URL" ]; then
    ...
    if [ "$ISSUE_STATE" == "CLOSED" ]; then
      echo "  Closing issue #$NEW_ISSUE_NUMBER to match original state..." >> "$LOG_FILE"
      gh issue close "$NEW_ISSUE_NUMBER" -R "$DEST_REPO"
    fi
  else
