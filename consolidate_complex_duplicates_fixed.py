        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cursor:
            for sec_mol in secondary_molecules:
                ...
                try:
                    ...
                    logger.info("Continuing with consolidation despite prediction migration error")
...
def migrate_mixture_components(cursor, source_id, target_id, consolidation_log):
    ...
    for component in mixture_components:
        ...
        # Handle properties - create a new properties object with consolidated_from
        properties_dict = {}
        if component.get('properties'):
            if isinstance(component['properties'], dict):
                properties_dict = component['properties'].copy()
            else:
                try:
                    properties_dict = json.loads(component['properties'])
                except:
                    properties_dict = {}
        
        # Add consolidated_from info
        properties_dict["consolidated_from"] = str(source_id)
        ...
        """, (
            ...
            Json(properties_dict), component.get('units')
        ))
...
def migrate_predictions(cursor, source_id, target_id, consolidation_log):
    ...
    try:
        ...
        return # Skip prediction migration if it fails
    ...
    for prediction in predictions:
        ...
        # Handle modification history - create a new modification history with consolidated_from
        mod_history_dict = {}
        if prediction.get('modification_history'):
            if isinstance(prediction['modification_history'], dict):
                mod_history_dict = prediction['modification_history'].copy()
            else:
                try:
                    mod_history_dict = json.loads(prediction['modification_history'])
                except:
                    mod_history_dict = {}
        ...
        """, (
            ...
            Json(mod_history_dict), prediction.get('name'), prediction.get('description'),
