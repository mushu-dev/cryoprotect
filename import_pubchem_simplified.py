#!/usr/bin/env python3
"""
CryoProtect PubChem Data Importer with Simplified Database Connection

This script imports cryoprotectant data from PubChem using the CID-Synonym-curated file
and stores it in the database using the new simplified database module. It includes:

1. Direct database connection for faster operations
2. Parallel processing for concurrent requests
3. Efficient batch operations with transactions
4. Enhanced checkpoint system

Usage:
    python import_pubchem_simplified.py [--batch-size BATCH_SIZE] [--api-delay API_DELAY] 
...
Parameters:
    ...
    --api-delay: Delay between PubChem API calls in seconds (default: 0.5)
    --target: Maximum number of compounds to import (default: 5000)
    --workers: Number of worker threads for parallel processing (default: 3)
...
# Set up logging
LOG_FILE = "logs/pubchem_import_simplified.log"
SKIPPED_CID_LOG = "logs/skipped_cids_simplified.log"
logging.basicConfig(
    level=logging.INFO,
...
CHECKPOINT_FILE = "checkpoints/pubchem_import_simplified.json"
...
def batch_insert_molecules(molecules):
    """
    Insert a batch of molecules into the database using transactions.
    
    Args:
        molecules: List of molecule data dictionaries
        
    Returns:
        Number of molecules successfully inserted
    """
    if not molecules:
        return 0
            ...
            for molecule in molecules:
                ...
                query = """
                    INSERT INTO molecules (
                        pubchem_cid, name, smiles, molecular_weight, formula, inchi, inchikey, 
                ...
                params = (
                    ...
                    molecule["created_at"],
                    molecule["created_at"]  # Use same timestamp for updated_at
                )
                ...
                if result and 'id' in result:
                    ...
                    insert_molecular_properties(cursor, result['id'], molecule)
            
            # Transaction automatically commits when the context manager exits
            logger.info(f"SUCCESS: Inserted {inserted_count} molecules in batch")
...
def insert_molecular_properties(cursor, molecule_id, molecule_data):
    """
    Insert molecular properties for a molecule.
    
    Args:
        cursor: Database cursor within a transaction
        molecule_id: ID of the molecule
        molecule_data: Molecule data dictionary with properties
    ...
    property_mapping = {
        "molecular_weight": ("Molecular Weight", "numeric"),
        "logp": ("LogP", "numeric"),
        "tpsa": ("TPSA", "numeric"),
        "h_bond_donors": ("H-Bond Donors", "numeric"),
        "h_bond_acceptors": ("H-Bond Acceptors", "numeric"),
        "iupac_name": ("IUPACName", "text")
    }
    
    inserted_count = 0
    
    # First get property type IDs
    for db_property, (mol_property, value_type) in property_mapping.items():
        if mol_property not in molecule_data or molecule_data[mol_property] is None:
            continue
        ...
        if result:
            ...
            cursor.execute(
                ...
                RETURNING id
                """,
                (db_property, value_type)
            )
            result = cursor.fetchone()
            property_type_id = result['id'] if result else None
            
        if not property_type_id:
            logger.warning(f"Could not create property type {db_property}")
            continue
            
        # Insert property value
        value = molecule_data[mol_property]

        # Use the correct column based on property type
        if value_type == "numeric":
            cursor.execute(
                ...
                ON CONFLICT (molecule_id, property_type_id) DO UPDATE SET
                    ...
                    updated_at = NOW()
                """,
                (molecule_id, property_type_id, value, "PubChem")
            )
        else:
            cursor.execute(
                """
                ...
                ON CONFLICT (molecule_id, property_type_id) DO UPDATE SET
                    ...
                    updated_at = NOW()
                """,
                (molecule_id, property_type_id, str(value), "PubChem")
            )
...
def load_checkpoint():
    ...
    try:
        ...
        if "position" in checkpoint:
            ...
            logger.info(f"Checkpoint loaded: Batch {last_batch}, {processed} processed")
            ...
            # Old format
            logger.info(f"Checkpoint loaded: Batch {checkpoint.get('last_completed_batch', 0)}, {checkpoint.get('total_processed', 0)} processed")
...
def generate_import_report(total_processed, total_imported, total_skipped, total_errors, elapsed_time, rate_limit_errors=0):
    ...
    report_file = f"reports/pubchem_import_report_simplified_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Import PubChem data into database using simplified database module.")
