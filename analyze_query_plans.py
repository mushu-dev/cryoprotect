#!/usr/bin/env python3
"""
CryoProtect v2 - Database Query Plan Analysis
...
QUERIES = [
    ...
    },
    {
        ...
        "query": "SELECT * FROM molecule_with_properties WHERE id = '{{molecule_id}}'"
    },
    {
        "name": "Get all mixtures with components",
        "query": "SELECT * FROM mixture_with_components LIMIT 100"
    },
    {
        "name": "Get mixture by ID",
        "query": "SELECT * FROM mixture_with_components WHERE id = '{{mixture_id}}'"
    },
    {
    ...
    },
    {
        ...
        "query": "SELECT * FROM molecule WHERE name ILIKE '%{{search_term}}%' LIMIT 100"
    },
    {
        "name": "Search mixtures by name",
        "query": "SELECT * FROM mixture WHERE name ILIKE '%{{search_term}}%' LIMIT 100"
    },
    {
        ...
        "query": """
            ...
            AND mp.value BETWEEN {{min_value}} AND {{max_value}}
            LIMIT 100
        """
    }
]
...
def get_test_data(supabase):
    ...
    # Get a molecule ID
    response = supabase.table("molecule").select("id").limit(1).execute()
    ...
    # Get a mixture ID
    response = supabase.table("mixture").select("id").limit(1).execute()
    if hasattr(response, 'data') and response.data:
...
def analyze_query_plan(supabase, query):
    ...
    try:
        ...
        response = supabase.rpc("execute_sql", {"query": explain_query}).execute()
...
def suggest_optimizations(query, plan, issues):
    ...
    # Check for ORDER BY and suggest indexes
    if "ORDER BY" in query.upper():
        # Extract ORDER BY columns
        import re
        order_matches = re.findall(r"ORDER BY\s+([^()]+)(?:\s+LIMIT|\s*$)", query, re.IGNORECASE)
        if order_matches:
            order_columns = order_matches[0].split(",")
            # Extract table name from the query
            from_matches = re.findall(r"FROM\s+(\w+)", query, re.IGNORECASE)
            if from_matches:
                table = from_matches[0]
                columns = [col.strip().split()[0] for col in order_columns]
                suggestions.append({
                    "type": "create_index",
                    "description": f"Create an index on {table}.{', '.join(columns)} to improve sorting performance.",
...
def main():
    ...
    # Save report to file
    with open("query_plan_analysis_report.json", "w") as f:
    ...
    with open("query_plan_analysis_report.txt", "w") as f:
        f.write("\n".join(text_report))
    
    logger.info("Query plan analysis completed. Reports saved to query_plan_analysis_report.json and query_plan_analysis_report.txt")
