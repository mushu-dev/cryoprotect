...
def check_dns(hostname):
    ...
    try:
        ...
        print(f"  {Colors.GREEN}[OK] Hostname resolved: {hostname} -> {ip_address}{Colors.ENDC}")
    except socket.gaierror as e:
        print(f"  {Colors.FAIL}[FAIL] Failed to resolve hostname: {hostname}{Colors.ENDC}")
        print(f"  Error: {str(e)}")
        ...
        try:
            ...
            print(f"  {Colors.GREEN}[OK] Base domain 'supabase.co' resolves to {base_ip}{Colors.ENDC}")
            print(f"  {Colors.WARNING}This suggests a specific project hostname issue, not a general DNS problem.{Colors.ENDC}")
        except socket.gaierror:
            print(f"  {Colors.FAIL}[FAIL] Failed to resolve even 'supabase.co'{Colors.ENDC}")
    ...
    try:
        ...
        print(f"  {Colors.GREEN}[OK] Database hostname resolved: {db_hostname} -> {db_ip_address}{Colors.ENDC}")
    except socket.gaierror as e:
        print(f"  {Colors.FAIL}[FAIL] Failed to resolve database hostname: {db_hostname}{Colors.ENDC}")
        print(f"  Error: {str(e)}")
...
def check_network(hostname):
    ...
    try:
        ...
        if response.status_code in [200, 204, 401, 403]:  # These status codes indicate network is fine
            print(f"  {Colors.GREEN}[OK] API is reachable (Status: {response.status_code}, Latency: {elapsed:.2f}s){Colors.ENDC}")
        else:
            print(f"  {Colors.WARNING}[WARNING] API returned unexpected status code: {response.status_code}{Colors.ENDC}")
            print(f"  Response: {response.text[:100]}")
    except requests.exceptions.RequestException as e:
        print(f"  {Colors.FAIL}[FAIL] Failed to connect to API: {str(e)}{Colors.ENDC}")
    ...
    try:
        ...
        if result == 0:
            print(f"  {Colors.GREEN}[OK] Database port is reachable{Colors.ENDC}")
        else:
            print(f"  {Colors.FAIL}[FAIL] Database port is not reachable (Error code: {result}){Colors.ENDC}")
        ...
        print(f"  {Colors.FAIL}[FAIL] Failed to test database connection: {str(e)}{Colors.ENDC}")
        return False
...
def check_auth():
    ...
    try:
        ...
        if response.status_code in [200, 204]:
            print(f"  {Colors.GREEN}[OK] Authentication successful{Colors.ENDC}")
            return True
        elif response.status_code == 401:
            print(f"  {Colors.FAIL}[FAIL] Authentication failed: Unauthorized (401){Colors.ENDC}")
            ...
            print(f"  {Colors.FAIL}[FAIL] Authentication failed: Forbidden (403){Colors.ENDC}")
            ...
            print(f"  {Colors.FAIL}[FAIL] Authentication failed: Unexpected status code {response.status_code}{Colors.ENDC}")
            print(f"  Response: {response.text[:100]}")
            return False
    except requests.exceptions.RequestException as e:
        print(f"  {Colors.FAIL}[FAIL] Failed to test authentication: {str(e)}{Colors.ENDC}")
        return False
...
def check_database():
    ...
    try:
        ...
        try:
            ...
            if result:
                print(f"  {Colors.GREEN}[OK] MCP SQL execution successful{Colors.ENDC}")
                print(f"  Result: {json.dumps(result, indent=2)[:100]}...")
                return True
            else:
                print(f"  {Colors.FAIL}[FAIL] MCP SQL execution failed: No result returned{Colors.ENDC}")
                return False
        except Exception as e:
            print(f"  {Colors.FAIL}[FAIL] MCP SQL execution failed: {str(e)}{Colors.ENDC}")
        ...
        print(f"  {Colors.WARNING}[WARNING] Could not import supabase_mcp_tools module. Skipping MCP test.{Colors.ENDC}")
        return None
...
print(f"  DNS Resolution:   {'[OK]' if dns_ok else '[FAIL]'}")
print(f"  Network Connectivity: {'[OK]' if network_ok else '[FAIL]'}")
print(f"  Authentication:   {'[OK]' if auth_ok else '[FAIL]'}")
print(f"  Database Access:  {'[OK]' if db_ok else '[N/A]' if db_ok is None else '[FAIL]'}")

print(f"\n{Colors.BOLD}Overall Status:{Colors.ENDC}")
if dns_ok and network_ok and auth_ok and (db_ok or db_ok is None):
    print(f"{Colors.GREEN}[OK] All checks passed! Your Supabase connection appears to be working correctly.{Colors.ENDC}")
    ...
    print(f"{Colors.FAIL}[FAIL] Some checks failed. See above for specific issues and recommended fixes.{Colors.ENDC}")
