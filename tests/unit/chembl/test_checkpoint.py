...
class TestCheckpointManager(unittest.TestCase):
    ...
    def test_cleanup_old_checkpoints(self):
        ...
        # Override the get_checkpoint_path method to create uniquely named files
        original_get_path = self.checkpoint_manager.get_checkpoint_path
        
        try:
            # Create a custom method that uses a counter for unique filenames
            def custom_get_path(counter):
                return os.path.join(self.temp_dir, f"chembl_import_test_{counter:02d}.json")
            
            # Create 10 checkpoints with unique names
            for i in range(10):
                # Override the method to return our custom path
                self.checkpoint_manager.get_checkpoint_path = lambda: custom_get_path(i)
                
                # Update state and save
                self.checkpoint_manager.state["total_processed"] = i
                checkpoint_path = self.checkpoint_manager.save_checkpoint()
                print(f"Created checkpoint {i+1}/10: {checkpoint_path}")
            ...
            checkpoint_files = [
                ...
                if f.startswith("chembl_import_test_") and f.endswith(".json")
            ]
            print(f"Found {len(checkpoint_files)} checkpoint files: {checkpoint_files}")
            
            # The test expects 5 files to be kept
            self.assertEqual(len(checkpoint_files), 5)
            
            # Verify we have the 5 most recent files (05-09)
            for i in range(5, 10):
                expected_file = f"chembl_import_test_{i:02d}.json"
                self.assertIn(expected_file, checkpoint_files)
                
        finally:
            # Restore the original method
            self.checkpoint_manager.get_checkpoint_path = original_get_path
