#!/usr/bin/env python3
"""
CryoProtect v2 - Simple Authentication Fix

This script creates the essential files needed to bypass the email confirmation requirement.
"""
...
print("CryoProtect v2 - Simple Authentication Fix")
print("----------------------------------------")
...
with open(".env.backup-simple", "w") as f:
    f.write(current_env)
    print("Created backup of .env file as .env.backup-simple")

# Create auth_config.py with hardcoded user ID
auth_config_content = """# Authentication Configuration
# This file is created by fix_auth_simple.py

# Hardcoded user ID from user creation
USER_ID = "748b5eb7-15dd-4019-b128-ae9d80d9d446"

# Whether to use the service role approach (bypass authentication)
...
print("Created auth_config.py with hardcoded user ID and service role flag")

# Create service_role_helper.py
helper_content = """#!/usr/bin/env python3
'''
CryoProtect v2 - Service Role Authentication Helper

This module provides helper functions for the service role authentication approach.
Include this in your scripts that need to bypass the email confirmation requirement.
'''
...
# Try to import auth_config, but don't fail if it doesn't exist
try:
    from auth_config import USER_ID, USE_SERVICE_ROLE
except ImportError:
    # Default values if auth_config.py doesn't exist
...
def ensure_user_profile(supabase=None):
    '''Ensure that a user profile exists for the current user ID.'''
    if supabase is None:
    ...
    profile_data = {
        ...
        "display_name": "CryoProtect User",
        "email": os.getenv("SUPABASE_USER", "user@example.com"),
    ...
    if hasattr(create_response, 'error') and create_response.error:
        raise Exception(f"Failed to create user profile: {create_response.error}")
    
    return profile_id
"""

with open("service_role_helper.py", "w") as f:
    f.write(helper_content)

print("Created service_role_helper.py for use in other scripts")

# Create a modified test_supabase_auth.py file
...
with open("test_service_role_auth.py", "w") as f:
    f.write(test_script_content)

print("Created test_service_role_auth.py to verify service role authentication")

print("\nSetup complete!")
print("\nNext steps:")
print("1. Run the test script to verify service role authentication:")
print("   python test_service_role_auth.py")
print("2. If the test is successful, you should be able to run the application")
print("3. To use the service role helper in your scripts, add:")
print("   from service_role_helper import get_supabase_client, get_user_id, ensure_user_profile")
