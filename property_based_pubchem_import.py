...
2. Property-based filtering for potential cryoprotectants
...
# Molecular property ranges for cryoprotectants
PROPERTY_FILTERS = [
        ...
        try:
            ...
            for name in property_names:
                if name not in existing:
                    ...
                    cursor.execute(
                        """
                        INSERT INTO property_types (id, name, created_at, updated_at)
                        VALUES (%s, %s, NOW(), NOW())
                        RETURNING id
                        """,
                        (property_id, name)
...
def get_compounds_by_properties(property_filter: Dict[str, str], delay: float = 0.5) -> List[int]:
    """
    Search for compounds that match specific property ranges.
    
    Args:
        property_filter: Dictionary of property names and range values
        delay: Delay between requests
        
    Returns:
        List of PubChem CIDs matching the properties
    """
    # Construct the property filter string
    filter_parts = []
    for prop, val_range in property_filter.items():
        filter_parts.append(f"{prop}={val_range}")
    
    # Join all property filters with commas
    filter_str = ",".join(filter_parts)
    
    # Create the property search URL
    search_url = f"{PUBCHEM_BASE_URL}/compound/fastsimilarity_2d/property/{filter_str}/JSON"
    
    logger.info(f"Searching compounds with property filter: {filter_str}")
    response = fetch_with_retry(search_url)
    cids = []
    ...
    if response and response.status_code == 200:
        ...
        cids = data.get("IdentifierList", {}).get("CID", [])
        if cids:
            logger.info(f"Found {len(cids)} compounds matching property filter")
        else:
            logger.info("No compounds found matching property filter")
    else:
        logger.warning(f"Property search failed: {property_filter}")
...
def get_similar_compounds(cid: int, threshold: float = 0.8, max_compounds: int = 100, delay: float = 0.5) -> List[int]:
    ...
    """
    search_url = f"{PUBCHEM_BASE_URL}/compound/similarity/cid/{cid}/JSON"
    params = {
        "Threshold": threshold,
...
def main():
    ...
    if args.checkpoint:
        ...
        # Step 2: Collect compounds based on property filters
        logger.info("Step 2: Collecting compounds based on property filters")
        for idx, filter_set in enumerate(PROPERTY_FILTERS, 1):
            logger.info(f"Applying property filter set {idx}: {filter_set}")
            filter_cids = get_compounds_by_properties(filter_set, delay=args.api_delay)
            logger.info(f"Found {len(filter_cids)} compounds matching filter set {idx}")
            all_cids.update(filter_cids)
        
        logger.info(f"Collected {len(all_cids)} compounds after property filtering")
        
        # Step 3: Find similar compounds to seed compounds
        if seed_cids:
            logger.info("Step 3: Finding similar compounds to seed compounds")
            similar_cids = set()
            
            # Use a subset of seed compounds if there are many
            selected_seeds = list(seed_cids)[:min(5, len(seed_cids))]
            for seed_cid in selected_seeds:
                similar = get_similar_compounds(seed_cid, threshold=args.similarity_threshold, max_compounds=20, delay=args.api_delay)
                similar_cids.update(similar)
                logger.info(f"Found {len(similar)} compounds similar to CID {seed_cid}")
