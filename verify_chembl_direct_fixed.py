#!/usr/bin/env python3
"""
Verify ChEMBL import data directly in Supabase database - Fixed Version.
...
def get_molecular_properties(conn, molecule_ids: List[str]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    # First, check if the method column exists in the molecular_properties table
    columns_query = """
    SELECT column_name
    FROM information_schema.columns
    ...
    query = f"""
    SELECT molecule_id, property_type, property_value
    {', calculation_method' if 'calculation_method' in columns else ''}
    {', method' if 'method' in columns else ''}
...
def generate_report(
    ...
    total_molecules: int,
    molecules_schema: List[Dict[str, Any]],
    molecular_properties_schema: List[Dict[str, Any]],
    ...
    report = {
        ...
        "database_info": {
            ...
            "molecules_schema": molecules_schema,
            "molecular_properties_schema": molecular_properties_schema,
...
def main():
    ...
    try:
        ...
        molecules_schema = get_table_schema(conn, 'molecules')
        molecular_properties_schema = get_table_schema(conn, 'molecular_properties')
        ...
        if chembl_molecules:
            ...
            if molecular_properties:
                first_molecule_id = next(iter(molecular_properties))
                properties = molecular_properties[first_molecule_id]
                if properties:
