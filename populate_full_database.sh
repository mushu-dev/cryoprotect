...
podman cp ./complete_pubchem_import.py $CONTAINER_NAME:/app/
...
# Run PubChem import
echo "Starting PubChem import..."
podman exec $CONTAINER_NAME bash -c "cd /app && python complete_pubchem_import.py --host '$HOST' --port '$PORT' --user '$USER' --password '$PASSWORD' --dbname '$DBNAME' --target $PUBCHEM_TARGET --api-delay 0.5 --batch-size 10"
