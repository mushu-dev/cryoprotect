#!/usr/bin/env python3
"""
Property-based PubChem data import script for CryoProtect with numeric_value handling.
...
This version fixes issues with property type handling and numeric_value constraints.
"""
...
class Database:
    ...
    def batch_insert_molecules(self, molecules: List[Dict[str, Any]]) -> Dict[str, str]:
        ...
        try:
            ...
            for molecule in molecules:
                # Ensure pubchem_cid is a string
                if isinstance(molecule.get('pubchem_cid'), int):
                    molecule['pubchem_cid'] = str(molecule['pubchem_cid'])
    ...
    def batch_insert_properties(self, property_data_list: List[Dict[str, Any]], property_types: Dict[str, str]) -> List[str]:
        """Insert multiple properties in a batch transaction with property type IDs and numeric_value for all numeric types"""
        connection = None
        try:
            connection = self.get_connection()
            cursor = connection.cursor(cursor_factory=RealDictCursor)
            ...
            for prop_data in property_data_list:
                ...
                # Handle property value based on data type
                property_value = prop_data.get('property_value')
                numeric_value = None
                text_value = None
                boolean_value = None
                
                # Convert property value to the appropriate type based on the property's data type
                property_data_type = PROPERTY_TYPE_MAPPING.get(property_name, {}).get('data_type')
                if property_data_type == 'numeric':
                    try:
                        numeric_value = float(property_value)
                    except (ValueError, TypeError):
                        logger.warning(f"Could not convert property {property_name} value '{property_value}' to numeric, skipping")
                        continue
                elif property_data_type == 'text':
                    text_value = str(property_value) if property_value is not None else None
                elif property_data_type == 'boolean':
                    if isinstance(property_value, bool):
                        boolean_value = property_value
                    elif isinstance(property_value, (int, float)):
                        boolean_value = bool(property_value)
                    elif isinstance(property_value, str):
                        boolean_value = property_value.lower() in ('true', 'yes', 't', 'y', '1')
                    else:
                        logger.warning(f"Could not convert property {property_name} value '{property_value}' to boolean, skipping")
                        continue
                ...
                if existing:
                    ...
                    """
                    cursor.execute(update_query, (
                        str(property_value) if property_value is not None else None,
                        numeric_value,
                        text_value,
                        boolean_value,
                        prop_data.get('units'),
                    ...
                    INSERT INTO molecular_properties (
                        ...
                        unit, source, property_type_id, created_at, updated_at
            ...
            if "PropertyTable" in data and "Properties" in data["PropertyTable"]:
                ...
                for api_name, display_name in property_mapping.items():
                    if api_name in prop_data:
                        ...
                        numeric_value = None
                        if PROPERTY_TYPE_MAPPING.get(display_name, {}).get('data_type') == 'numeric':
                            try:
                                numeric_value = float(prop_data[api_name])
                            except (ValueError, TypeError):
                                logger.warning(f"Could not convert {api_name} value to numeric")
                                continue
                            
                        result.append({
                            "property_name": display_name,
                            "property_value": prop_data[api_name],
                            "numeric_value": numeric_value,  # Include numeric_value for constraint
                ...
                for molecule in batch_molecules:
                    ...
                    if cid in molecule_map:
                        ...
                        for prop in additional_properties:
                            ...
                            # Ensure numeric_value is set for numeric properties
                            if PROPERTY_TYPE_MAPPING.get(prop["property_name"], {}).get("data_type") == "numeric":
                                try:
                                    prop["numeric_value"] = float(prop["property_value"])
                                except (ValueError, TypeError):
                                    logger.warning(f"Could not convert property {prop['property_name']} value '{prop['property_value']}' to numeric")
