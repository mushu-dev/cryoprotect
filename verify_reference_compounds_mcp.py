#!/usr/bin/env python3
"""
Reference Compound Verification Script using MCP
...
This version uses the connected Supabase MCP server instead of direct npx commands.
"""
...
# Import the reference compounds module
...
class ReferenceCompoundVerifier:
    ...
    def execute_sql(self, query):
        """Execute SQL query using the Supabase MCP server"""
        try:
            ...
            cmd = [
                "C:\\Users\\1edwa\\Documents\\CryoProtect v2\\.venv\\Scripts\\python.exe",
                "-c",
                f"""
import json
import sys
...
try:
    ...
    result = execute_sql(query, '{self.project_id}' if '{self.project_id}' else None)
    ...
    print(json.dumps({{"error": str(e)}}))
            ...
            if result.returncode == 0 and result.stdout:
                try:
                    ...
                    return {"error": f"Failed to parse query result: {result.stdout}"}
            else:
                logger.error(f"Error executing query: {result.stderr}")
                return {"error": f"Failed to execute query: {result.stderr}"}
    ...
    def verify_reference_compounds(self):
        ...
        try:
            ...
            for chembl_id in reference_ids:
                ...
                if "error" in result:
                    logger.error(f"Error checking reference compound {chembl_id}: {result['error']}")
    ...
    def check_critical_properties(self):
        ...
        try:
            ...
            if "error" in result:
                logger.error(f"Error checking critical properties: {result['error']}")
            ...
            if result and len(result) > 0:
                ...
                total_result = self.execute_sql(total_query)
                
                if "error" in total_result:
                    logger.error(f"Error getting total molecule count: {total_result['error']}")
                    total_count = 0
                else:
                ...
                if self.results["reference_compounds"]:
                    ...
                    key_result = self.execute_sql(key_query)
                    
                    if "error" in key_result:
                        logger.error(f"Error checking key reference compounds: {key_result['error']}")
                    elif key_result and len(key_result) > 0:
