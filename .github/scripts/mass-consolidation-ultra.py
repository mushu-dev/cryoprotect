#!/usr/bin/env python3
"""
Ultra-Aggressive Mass Issue Consolidation Script

This script extremely aggressively consolidates GitHub issues by:
1. Creating 9 well-structured epic issues
2. Extracting key information from all existing issues 
3. Keeping only the absolute minimum number of issues
4. Closing ALL other issues without exception

Goal: Reduce issue count from 240+ to under 50

Usage:
    python mass-consolidation-ultra.py [--execute] [--quiet]
"""
...
# ONLY keep these specific issues (adjust numbers as needed)
KEEP_ISSUE_NUMBERS = [
    246,  # Phase 5: User Experience and Deployment Implementation
    245,  # Comprehensive RDKit and Database Optimization Plan
    244,  # GitHub Issue Organization Implementation
    243,  # Implement unified ChEMBL and PubChem data import pipeline
...
def main():
    ...
    for issue in all_issues:
        ...
        if issue["number"] in KEEP_ISSUE_NUMBERS:
            kept_issues.append(issue)
            continue
        
        # Categorize all other issues
        categories = categorize_issue(issue)
        categorized_issues[categories[0]].append(issue)
    
    # Print summary
    print("\n=== CATEGORIZATION SUMMARY ===")
    print(f"Issues to keep unchanged: {len(kept_issues)}")
    print("\nIssues by category:")
    ...
    total_kept = len(kept_issues)
    consolidated_count = total_epics + total_kept
    
    print(f"\nAfter consolidation: {consolidated_count} issues")
    print(f"  {total_epics} epic issues")
    print(f"  {total_kept} kept unchanged issues")
    ...
    for cat, info in EPIC_CATEGORIES.items():
        ...
        if cat in categorized_issues:
            ...
            body += "No issues have been categorized to this epic yet.\n\n"
    ...
    # Step 3: Close all issues except those in the keep list
    closed_count = 0
    
    for cat, issues in categorized_issues.items():
        if cat not in epic_numbers:
            epic_reference = "appropriate epic"
        else:
            epic_reference = f"epic #{epic_numbers[cat]}"
            
        for issue in issues:
            # Skip issues in keep list
            if issue["number"] in KEEP_ISSUE_NUMBERS:
                continue
                
            issue_num = issue["number"]
            print(f"Closing issue #{issue_num} (consolidated into {epic_reference})...")
            close_issue(
                issue_num, 
                f"This issue has been consolidated into {epic_reference} as part of repository cleanup and organization.\n\n"
