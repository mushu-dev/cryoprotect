#!/bin/bash
# run_database_integrity_check.sh
#
# Runs the enhanced database integrity verification script and generates a
...
echo "CryoProtect Database Integrity Verification"
echo "==========================================="
...
python verify_database_integrity_enhanced.py --output="$REPORT_FILE" $VERBOSE
...
if [ -f "$REPORT_FILE" ]; then
  ...
  if command -v jq &> /dev/null; then
    jq -r '.issues[] | "### \(.title)\n**Severity**: \(.severity)\n\n\(.description)\n\n---\n"' "$REPORT_FILE" >> "$MD_REPORT" 2>/dev/null || 
      echo "Failed to extract issues details" >> "$MD_REPORT"
  else
